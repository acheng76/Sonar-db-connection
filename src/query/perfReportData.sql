...
WITH SLA AS (
    SELECT
        ...
        PROD_EXTRNL_ALIAS_EN_NM AS PROD_EXTRNL_ALIAS_NM
    FROM
    ...
    WHERE
        perf_fact.SAMP_CUST_RQST_TS BETWEEN TO_DATE (
            :startTs,
...
hierarchical_org_with AS (
    SELECT
        ...
        (
            CASE
                WHEN UPPER (CUST_ORG_LEVEL_TYP) = 'AGENCY' THEN CUST_ORG_EN_NM
            END
        ) AS AGENCY,
        (
            CASE
                WHEN UPPER (CUST_ORG_LEVEL_TYP) = 'MINISTRY' THEN CUST_ORG_EN_NM
...
SELECT
    ...
    (
        CASE
            WHEN SLA_STAT_TYP_CD IS NULL
            OR UPPER (SLA_STAT_TYP_CD) = 'N/A' THEN 'N/A'
            WHEN UPPER (SLA_STAT_TYP_CD) = 'PASSED' THEN 'Y'
            WHEN UPPER (SLA_STAT_TYP_CD) = 'FAILED' THEN 'N'
            WHEN UPPER (SLA_STAT_TYP_CD) = 'FAILED-OMISSION' THEN 'O'
            WHEN UPPER (SLA_STAT_TYP_CD) = 'EXCLUDED-UTILIZ' THEN 'U'
            WHEN UPPER (SLA_STAT_TYP_CD) = 'EXCLUDED-NOT-REACH' THEN 'U'
            WHEN UPPER (SLA_STAT_TYP_CD) = 'EXCLUDED-INCIDENT' THEN 'A'
            ELSE 'N/A'
        END
    ) AS SLT_INDICATOR,
    '' AS SRC_MSU_ID,
